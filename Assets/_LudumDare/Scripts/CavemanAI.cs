...
public class CavemanAI : MonoBehaviour
{
    ...
    private bool startTutorialTrigger = false;


    void Start()
    {

        //List of actions needed to complete the game
    ...
    {
        if (startTutorialTrigger == false)
        { 
            //go into crouch animation
            manAnim.SetTrigger("pickUp");
            startTutorialTrigger = true;
        }
    ...
    private void EndUpdate()
    {
        if (InputSystem.WoofInput.IsEmote == true && InputSystem.WoofInput.IsBark == true && CheckList.ShitList.isMeatCooked == true)
        {
            ...
            ScreenFade.SFade.AudioFadeOut(audioSource, audioSource.volume);
        }
    }
